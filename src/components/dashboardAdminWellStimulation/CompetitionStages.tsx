...
const CompetitionStages = () => {
    ...
    const [wellStimulationCompetition, setWellStimulationCompetition] =
    ...
    const isWellStimulationCompetition = (name?: string) => {
        const normalizedName = name?.toLowerCase().replace(/\s+/g, "") || ""

        return (
            normalizedName.includes("wellstimulation") ||
            normalizedName.includes("well-stimulation") ||
            normalizedName.includes("well_stimulation")
        )
    }
    ...
    useEffect(() => {
        fetchWellStimulationCompetition()
        fetchTeams()
        fetchRegistrations()
    }, [])
    ...
    const fetchWellStimulationCompetition = async () => {
        try {
            setIsLoading(true)
            ...
            const wellStimulation = competitions.find((competition) =>
                isWellStimulationCompetition(competition.name_competition)
            )
            ...
            setWellStimulationCompetition(wellStimulation)
            fetchStagesByCompetition(wellStimulation.id_competition)
        } catch (error) {
            console.error(error)
            setToast({
                message: "Failed to fetch Well Stimulation competition.",
                type: "error",
            })
        } finally {
            setIsLoading(false)
    ...
    const filteredTeams = useMemo(() => {
        ...
        const wellStimulationRegistrations = registrations.filter(
            (registration) =>
                Number(registration.id_competition) ===
                Number(wellStimulationCompetition.id_competition)
        )

        const registeredTeamLeaderIds = wellStimulationRegistrations
    ...
    }, [teams, registrations, wellStimulationCompetition])
    ...
    const handleSubmitStage = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setEditingStage(null)
            fetchStagesByCompetition(wellStimulationCompetition.id_competition)
        } catch (error) {
            console.error(error)
            setToast({
    ...
    const handleUpdatePassedTeams = async () => {
        if (!selectedStage || !wellStimulationCompetition) return
        ...
        <div className="text-white space-y-6 px-10 py-7">
            <div>
                <p className="text-2xl font-bold italic">
                    Well Stimulation Stage Management
                </p>
                <p className="text-sm opacity-70 mt-1">
                    Manage stages, schedules, status, and passed teams for Well
            ...
            <div className="glass p-5 rounded-2xl">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-5">
                    ...
                    <button
                        ...
                        disabled={!wellStimulationCompetition}
                ...
                <div className="overflow-x-auto">
                    ...
                    {!isLoading && stages.length === 0 && (
                        <p className="text-center text-sm opacity-70 py-6">
                            No stage data found for Well Stimulation Competition.
                        </p>
                    )}
            ...
            {isStageFormModalOpen && wellStimulationCompetition && (
                ...
                <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-9999 px-5">
                    <div className="glass p-6 rounded-2xl w-full max-w-4xl max-h-[85vh] overflow-y-auto">
                        ...
                        {filteredTeams.length === 0 && (
                            <p className="text-center text-sm opacity-70 py-6">
                                No team data found for Well Stimulation
                                Competition.
                            </p>
                        )}
