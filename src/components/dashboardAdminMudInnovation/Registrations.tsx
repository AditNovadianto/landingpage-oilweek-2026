...
const Registrations = () => {
    ...
    const getTeamByRegistration = (registration: Registration) => {
        if (registration.id_team) {
            return teams.find(
                (team) => Number(team.id_team) === Number(registration.id_team)
            )
        }
    ...
    const getTeamName = (registration: Registration) => {
        const team = getTeamByRegistration(registration)
    ...
    const getTeamLeaderEmail = (registration: Registration) => {
        ...
        return registration.email_team_leader || teamLeader?.email_team_leader || "-"
    }

    const filteredRegistrations = useMemo(() => {
        const keyword = search.toLowerCase()

        return mudInnovationRegistrations.filter((registration) => {
            return (
                registration.category_registration
                    ?.toLowerCase()
                    .includes(keyword) ||
                getTeamName(registration).toLowerCase().includes(keyword) ||
                getTeamLeaderName(registration)
                    .toLowerCase()
                    .includes(keyword) ||
                getTeamLeaderEmail(registration).toLowerCase().includes(keyword)
            )
        })
    }, [mudInnovationRegistrations, search, teams, teamLeaders])

    const approvedPayments = mudInnovationRegistrations.filter(
        (item) => item.payment_status === "APPROVED"
    ).length

    const pendingPayments = mudInnovationRegistrations.filter(
        (item) => item.payment_status === "PENDING"
    ).length

    const activeRegistrations = mudInnovationRegistrations.filter(
        ...
        <div className="text-white space-y-6 px-10 py-7">
            <div>
                <p className="text-2xl font-bold italic">
                    Mud Innovation Registration Management
                </p>
                <p className="text-sm opacity-70 mt-1">
                    Manage registrations and verify payment status for Mud
                    Innovation Competition only.
                </p>
            </div>
            ...
            <div className="glass p-5 rounded-2xl">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-5">
                    <div>
                        <p className="font-semibold text-lg">
                            Mud Innovation Registration List
                        </p>
                        <p className="text-sm opacity-70">
                            List of team leaders who registered for Mud
                ...
                <div className="overflow-x-auto">
                    ...
                    {!isLoading && filteredRegistrations.length === 0 && (
                        <p className="text-center text-sm opacity-70 py-6">
                            No Mud Innovation registration data found.
                        </p>
                    )}
                </div>
            </div>
