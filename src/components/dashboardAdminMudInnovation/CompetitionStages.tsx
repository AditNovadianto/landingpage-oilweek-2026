...
const CompetitionStages = () => {
    ...
    const [mudInnovationCompetition, setMudInnovationCompetition] =
    ...
    const fetchMudInnovationCompetition = async () => {
        try {
            setIsLoading(true)
            ...
            if (!mudInnovation) {
                setToast({
                    message: "Mud Innovation competition not found.",
                    type: "error",
                })
                return
            }

            setMudInnovationCompetition(mudInnovation)
            fetchStagesByCompetition(mudInnovation.id_competition)
        } catch (error) {
            console.error(error)
            setToast({
                message: "Failed to fetch Mud Innovation competition.",
                type: "error",
            })
        } finally {
            setIsLoading(false)
    ...
    const filteredTeams = useMemo(() => {
        ...
        const mudInnovationRegistrations = registrations.filter(
            (registration) =>
                Number(registration.id_competition) ===
                Number(mudInnovationCompetition.id_competition)
        )

        const registeredTeamLeaderIds = mudInnovationRegistrations
    ...
    }, [teams, registrations, mudInnovationCompetition])
    ...
    const handleSubmitStage = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const body = editingStage
                ? {
                    id_competition:
                        mudInnovationCompetition.id_competition,
    ...
    const handleUpdatePassedTeams = async () => {
        if (!selectedStage || !mudInnovationCompetition) return
        ...
        <div className="text-white space-y-6 px-10 py-7">
            <div>
                <p className="text-2xl font-bold italic">
                    Mud Innovation Stage Management
                </p>
                <p className="text-sm opacity-70 mt-1">
                    Manage stages, schedules, status, and passed teams for Mud
            ...
            <div className="glass p-5 rounded-2xl">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-5">
                    ...
                    <button
                        ...
                        disabled={!mudInnovationCompetition}
                ...
                <div className="overflow-x-auto">
                    ...
                    {!isLoading && stages.length === 0 && (
                        <p className="text-center text-sm opacity-70 py-6">
                            No stage data found for Mud Innovation Competition.
                        </p>
                    )}
            ...
            {isStageFormModalOpen && mudInnovationCompetition && (
                ...
                <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-9999 px-5">
                    <div className="glass p-6 rounded-2xl w-full max-w-4xl max-h-[85vh] overflow-y-auto">
                        ...
                        {filteredTeams.length === 0 && (
                            <p className="text-center text-sm opacity-70 py-6">
                                No team data found for Mud Innovation
                                Competition.
                            </p>
                        )}
