...
const Home = () => {
    ...
    const BUSINESS_CASE_KEYWORD = "business case"
    ...
    const businessCaseCompetition = useMemo(() => {
        return competitions.find((competition) =>
            competition.name_competition
                ?.toLowerCase()
                .includes(BUSINESS_CASE_KEYWORD)
        )
    }, [competitions])

    const businessCaseRegistrations = useMemo(() => {
        if (!businessCaseCompetition) return []

        return registrations.filter(
            (registration) =>
                registration.id_competition ===
                businessCaseCompetition.id_competition
        )
    }, [registrations, businessCaseCompetition])

    const businessCaseTeams = useMemo(() => {
        const leaderIds = businessCaseRegistrations
    ...
    }, [teams, businessCaseRegistrations])

    const businessCaseTeamLeaders = useMemo(() => {
        const leaderIds = businessCaseRegistrations
            .map((registration) => Number(registration.id_team_leader))
            .filter(Boolean)
    ...
    }, [teamLeaders, businessCaseRegistrations])

    const businessCaseMembers = useMemo(() => {
        const teamIds = businessCaseTeams.map((team) => team.id_team)

        return members.filter((member) => teamIds.includes(Number(member.id_team)))
    }, [members, businessCaseTeams])

    const memberByTeam = useMemo(() => {
        return businessCaseTeams.map((team) => {
            const totalMember = businessCaseMembers.filter(
    ...
    }, [businessCaseTeams, businessCaseMembers])
    ...
    const avgMemberPerTeam =
        businessCaseTeams.length > 0
            ? (businessCaseMembers.length / businessCaseTeams.length).toFixed(1)
            : "0"

    const stats = [
        {
            title: "Business Case Competition",
            value: businessCaseCompetition ? 1 : 0,
            icon: Trophy,
        },
        {
            title: "Total Team",
            value: businessCaseTeams.length,
            icon: Users,
        },
        {
            title: "Total Team Leader",
            value: businessCaseTeamLeaders.length,
            icon: UserRound,
        },
        {
            title: "Total Registration",
            value: businessCaseRegistrations.length,
            icon: ClipboardList,
        },
        {
            title: "Total Member",
            value: businessCaseMembers.length,
            icon: UserCheck,
        },
    ]
        ...
        <div className="text-white space-y-6 px-10 py-7">
            <div>
                <p className="text-2xl font-bold italic">
                    Dashboard Admin Business Case Competition
                </p>
                <p className="text-sm opacity-70 mt-1">
                    Monitor team, team leader, registration, and member data for
                    Business Case Competition only.
                </p>
            </div>
            ...
            {!businessCaseCompetition && (
                <div className="glass p-5 rounded-2xl">
                    <p className="text-sm text-yellow-300">
                        Data Business Case Competition belum ditemukan. Pastikan
                        nama competition mengandung kata "Business Case".
                    </p>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                <div className="glass p-5 rounded-2xl lg:col-span-2">
                    <div className="flex items-center justify-between mb-5">
                        <div>
                            ...
                            <p className="text-sm opacity-70">
                                Total member pada masing-masing team Business Case
                            </p>
                        </div>
                    ...
                    <div className="space-y-4">
                        {memberByTeam.length > 0 ? (
                            ...
                            <p className="text-sm opacity-70">
                                Belum ada team yang terdaftar di Business Case
                                Competition.
                            </p>
                        )}
                    </div>
                ...
                <div className="glass p-5 rounded-2xl">
                    ...
                    <p className="text-sm opacity-70 mb-5">
                        Ringkasan pendaftaran khusus Business Case Competition
                    </p>
                        ...
                        <div className="relative w-44 h-44 rounded-full glass flex items-center justify-center">
                            <div className="text-center">
                                <p className="text-4xl font-bold">
                                    {businessCaseRegistrations.length}
                                </p>
                                <p className="text-sm opacity-70">
                                    Registrations
                                </p>
                    ...
                    <div className="mt-5 space-y-3 text-sm">
                        <div className="flex justify-between">
                            <p>Total Team</p>
                            <p>{businessCaseTeams.length}</p>
                        </div>

                        <div className="flex justify-between">
                            <p>Total Team Leader</p>
                            <p>{businessCaseTeamLeaders.length}</p>
                        </div>
                ...
                <div className="overflow-x-auto">
                    ...
                    {memberByTeam.length === 0 && (
                        <p className="text-center text-sm opacity-70 py-5">
                            Belum ada data team untuk Business Case Competition.
                        </p>
                    )}
                </div>
            </div>
