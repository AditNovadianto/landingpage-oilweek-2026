...
const Registrations = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            setTeams(
                normalizeArray<Team>(teamData, "teams")
            )
        } catch (error) {
            console.error(error)
            setToast({
    ...
    const filteredRegistrations = useMemo(() => {
        ...
        return businessCaseRegistrations.filter((registration) => {
            return (
    ...
    }, [businessCaseRegistrations, search])

    const approvedPayments = businessCaseRegistrations.filter(
        (item) => item.payment_status === "APPROVED"
    ).length

    const pendingPayments = businessCaseRegistrations.filter(
        (item) => item.payment_status === "PENDING"
    ).length

    const activeRegistrations = businessCaseRegistrations.filter(
    ...
    const handleUpdatePaymentStatus = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const res = await fetch(
                `${import.meta.env.VITE_API_BASE_URL
                }/updateRegistration/${selectedRegistration.id_registration}`,
                {
                    method: "PUT",
                    headers: {
        ...
        <div className="text-white space-y-6 px-10 py-7">
            <div>
                <p className="text-2xl font-bold italic">
                    Business Case Registration Management
                </p>
                <p className="text-sm opacity-70 mt-1">
                    Manage registrations and verify payment status for Business
                    Case Competition only.
                </p>
            </div>
            ...
            <div className="glass p-5 rounded-2xl">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-5">
                    <div>
                        <p className="font-semibold text-lg">
                            Business Case Registration List
                        </p>
                        <p className="text-sm opacity-70">
                            List of team leaders who registered for Business
                            Case Competition.
                        </p>
                    </div>
                ...
                <div className="overflow-x-auto">
                            ...
                            {filteredRegistrations.map(
                                (registration, index) => (
                                    ...
                                    >
                                        ...
                                        <td className="py-3">
                                            {teams.find((team) => team.id_team_leader === registration.id_team_leader)?.team_name}
                                        </td>
                    ...
                    {!isLoading && filteredRegistrations.length === 0 && (
                        <p className="text-center text-sm opacity-70 py-6">
                            No Business Case registration data found.
                        </p>
                    )}
                </div>
            </div>
                    ...
                    <div className="glass p-6 rounded-2xl h-[95%] w-full max-w-lg overflow-y-auto">
                        ...
                        <div className="space-y-3 text-sm">
                            ...
                            <DetailRow
                                label="Team"
                                value={
                                    teams.find((team) => team.id_team_leader === selectedRegistration.id_team_leader)?.team_name
                                }
                            />
                            <DetailRow
                                label="Team Leader"
                            ...
                            <DetailRow
                                ...
                                value={
                                    selectedRegistration.email_team_leader ||
                                    teamLeaders.find(
                                        (teamLeader) =>
                                            teamLeader.id_team_leader ===
                                            selectedRegistration.id_team_leader
                        ...
                        <form onSubmit={handleUpdatePaymentStatus}>
                            <div className="space-y-3 text-sm mb-5">
                                <DetailRow
                                    label="Team"
                                    value={
                                        selectedRegistration.name_team ||
                                        selectedRegistration.id_team ||
                                        "-"
                                    }
                                />
                                <DetailRow
