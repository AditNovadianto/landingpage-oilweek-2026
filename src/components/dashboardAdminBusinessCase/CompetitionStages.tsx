...
const CompetitionStages = () => {
    ...
    const [businessCaseCompetition, setBusinessCaseCompetition] =
    ...
    const fetchBusinessCaseCompetition = async () => {
        try {
            setIsLoading(true)
            ...
            const businessCase = competitions.find((competition) =>
            ...
            if (!businessCase) {
                setToast({
                    message: "Business Case competition not found.",
                    type: "error",
                })
                return
            }

            setBusinessCaseCompetition(businessCase)
            fetchStagesByCompetition(businessCase.id_competition)
        } catch (error) {
            console.error(error)
            setToast({
                message: "Failed to fetch Business Case competition.",
                type: "error",
            })
        } finally {
            setIsLoading(false)
    ...
    const filteredTeams = useMemo(() => {
        if (!businessCaseCompetition) return []

        const businessCaseRegistrations = registrations.filter(
            (registration) =>
                Number(registration.id_competition) ===
                Number(businessCaseCompetition.id_competition)
        )

        const registeredTeamLeaderIds = businessCaseRegistrations
    ...
    }, [teams, registrations, businessCaseCompetition])
    ...
    const handleSubmitStage = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const body = editingStage
                ? {
                    id_competition: businessCaseCompetition.id_competition,
            ...
            setEditingStage(null)
            fetchStagesByCompetition(businessCaseCompetition.id_competition)
        } catch (error) {
            console.error(error)
            setToast({
    ...
    const handleUpdatePassedTeams = async () => {
        if (!selectedStage || !businessCaseCompetition) return
        ...
        <div className="text-white space-y-6 px-10 py-7">
            <div>
                <p className="text-2xl font-bold italic">
                    Business Case Stage Management
                </p>
                <p className="text-sm opacity-70 mt-1">
                    Manage stages, schedules, status, and passed teams for
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="glass p-5 rounded-2xl">
                    ...
                    <p className="text-xl font-bold mt-1">
                        {businessCaseCompetition?.name_competition || "-"}
                    </p>
                </div>
            ...
            <div className="glass p-5 rounded-2xl">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-5">
                    ...
                    <button
                        ...
                        disabled={!businessCaseCompetition}
                ...
                <div className="overflow-x-auto">
                    ...
                    {!isLoading && stages.length === 0 && (
                        <p className="text-center text-sm opacity-70 py-6">
                            No stage data found for Business Case Competition.
                        </p>
                    )}
            ...
            {isStageFormModalOpen && businessCaseCompetition && (
                ...
                <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-9999 px-5">
                    <div className="glass p-6 rounded-2xl w-full max-w-4xl max-h-[85vh] overflow-y-auto">
                        ...
                        {filteredTeams.length === 0 && (
                            <p className="text-center text-sm opacity-70 py-6">
                                No team data found for Business Case
                                Competition.
                            </p>
                        )}
