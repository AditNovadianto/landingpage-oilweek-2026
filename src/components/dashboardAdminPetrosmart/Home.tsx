...
const Home = () => {
    ...
    const petrosmartRegistrations = useMemo(() => {
        ...
        return registrations.filter(
            (registration) =>
                ...
                Number(petrosmartCompetition.id_competition)
        )
    }, [registrations, petrosmartCompetition])

    const petrosmartTeams = useMemo(() => {
        const leaderIds = petrosmartRegistrations
    ...
    }, [teams, petrosmartRegistrations])

    const petrosmartTeamLeaders = useMemo(() => {
        const leaderIds = petrosmartRegistrations
            .map((registration) => Number(registration.id_team_leader))
            .filter(Boolean)
    ...
    }, [teamLeaders, petrosmartRegistrations])

    const petrosmartMembers = useMemo(() => {
        const teamIds = petrosmartTeams.map((team) => team.id_team)

        return members.filter((member) => teamIds.includes(Number(member.id_team)))
    }, [members, petrosmartTeams])

    const memberByTeam = useMemo(() => {
        return petrosmartTeams.map((team) => {
            const totalMember = petrosmartMembers.filter(
    ...
    }, [petrosmartTeams, petrosmartMembers])
    ...
    const avgMemberPerTeam =
        petrosmartTeams.length > 0
            ? (petrosmartMembers.length / petrosmartTeams.length).toFixed(1)
            : "0"

    const stats = [
        {
            title: "Petrosmart Competition",
            value: petrosmartCompetition ? 1 : 0,
            icon: Trophy,
        },
        {
            title: "Total Team",
            value: petrosmartTeams.length,
            icon: Users,
        },
        {
            title: "Total Team Leader",
            value: petrosmartTeamLeaders.length,
            icon: UserRound,
        },
        {
            title: "Total Registration",
            value: petrosmartRegistrations.length,
            icon: ClipboardList,
        },
        {
            title: "Total Member",
            value: petrosmartMembers.length,
            icon: UserCheck,
        },
    ]
        ...
        <div className="text-white space-y-6 px-10 py-7">
            <div>
                <p className="text-2xl font-bold italic">
                    Dashboard Admin Petrosmart Competition
                </p>
            ...
            {!petrosmartCompetition && (
                <div className="glass p-5 rounded-2xl">
                    <p className="text-sm text-yellow-300">
                        Data Petrosmart Competition belum ditemukan. Pastikan
                        nama competition mengandung kata "Petrosmart".
                    </p>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                <div className="glass p-5 rounded-2xl lg:col-span-2">
                    <div className="flex items-center justify-between mb-5">
                        <div>
                            ...
                            <p className="text-sm opacity-70">
                                Total member pada masing-masing team Petrosmart
                            </p>
                        </div>
                    ...
                    <div className="space-y-4">
                        {memberByTeam.length > 0 ? (
                            ...
                            <p className="text-sm opacity-70">
                                Belum ada team yang terdaftar di Petrosmart
                                Competition.
                            </p>
                        )}
                    </div>
                ...
                <div className="glass p-5 rounded-2xl">
                    ...
                    <p className="text-sm opacity-70 mb-5">
                        Ringkasan pendaftaran khusus Petrosmart Competition
                    </p>
                    ...
                    <div className="mt-5 space-y-3 text-sm">
                        <div className="flex justify-between">
                            <p>Total Team</p>
                            <p>{petrosmartTeams.length}</p>
                        </div>

                        <div className="flex justify-between">
                            <p>Total Team Leader</p>
                            <p>{petrosmartTeamLeaders.length}</p>
                        </div>
                ...
                <div className="overflow-x-auto">
                    ...
                    {memberByTeam.length === 0 && (
                        <p className="text-center text-sm opacity-70 py-5">
                            Belum ada data team untuk Petrosmart Competition.
                        </p>
                    )}
                </div>
            </div>
