...
const CompetitionStages = () => {
    ...
    const [caseStudyCompetition, setCaseStudyCompetition] =
    ...
    const fetchCaseStudyCompetition = async () => {
        try {
            ...
            const caseStudy = competitions.find((competition) =>
            ...
            if (!caseStudy) {
                setToast({
                    message: "Case Study competition not found.",
                    type: "error",
                })
                return
            }

            setCaseStudyCompetition(caseStudy)
            fetchStagesByCompetition(caseStudy.id_competition)
        } catch (error) {
            console.error(error)
            setToast({
                message: "Failed to fetch Case Study competition.",
                type: "error",
            })
        } finally {
            setIsLoading(false)
    ...
    const filteredTeams = useMemo(() => {
        ...
        const caseStudyRegistrations = registrations.filter(
            (registration) =>
                Number(registration.id_competition) ===
                Number(caseStudyCompetition.id_competition)
        )

        const registeredTeamLeaderIds = caseStudyRegistrations
    ...
    }, [teams, registrations, caseStudyCompetition])
    ...
    const handleSubmitStage = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const body = editingStage
                ? {
                    id_competition: caseStudyCompetition.id_competition,
            ...
            setEditingStage(null)
            fetchStagesByCompetition(caseStudyCompetition.id_competition)
        } catch (error) {
            console.error(error)
            setToast({
    ...
    const handleUpdatePassedTeams = async () => {
        if (!selectedStage || !caseStudyCompetition) return
        ...
        <div className="text-white space-y-6 px-10 py-7">
            <div>
                <p className="text-2xl font-bold italic">
                    Case Study Stage Management
                </p>
                <p className="text-sm opacity-70 mt-1">
                    Manage stages, schedules, status, and passed teams for Case
                    Study Competition.
                </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="glass p-5 rounded-2xl">
                    ...
                    <p className="text-xl font-bold mt-1">
                        {caseStudyCompetition?.name_competition || "-"}
                    </p>
                </div>
                ...
                <div className="overflow-x-auto">
                    ...
                    {!isLoading && stages.length === 0 && (
                        <p className="text-center text-sm opacity-70 py-6">
                            No stage data found for Case Study Competition.
                        </p>
                    )}
            ...
            {isStageFormModalOpen && caseStudyCompetition && (
                ...
                <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-9999 px-5">
                    <div className="glass p-6 rounded-2xl w-full max-w-4xl max-h-[85vh] overflow-y-auto">
                        ...
                        {filteredTeams.length === 0 && (
                            <p className="text-center text-sm opacity-70 py-6">
                                No team data found for Case Study Competition.
                            </p>
                        )}
