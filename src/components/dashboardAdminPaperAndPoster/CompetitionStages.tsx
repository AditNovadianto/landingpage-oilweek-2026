...
const CompetitionStages = () => {
    ...
    const [paperAndPosterCompetition, setPaperAndPosterCompetition] =
    ...
    const isPaperAndPosterCompetition = (name?: string) => {
        const normalizedName = name?.toLowerCase().replace(/\s+/g, "") || ""

        return (
            normalizedName.includes("paper&poster") ||
            normalizedName.includes("paperandposter") ||
            normalizedName.includes("paperposter")
        )
    }
    ...
    useEffect(() => {
        fetchPaperAndPosterCompetition()
        fetchTeams()
        fetchRegistrations()
    }, [])
    ...
    const fetchPaperAndPosterCompetition = async () => {
        try {
            setIsLoading(true)
            ...
            const paperAndPoster = competitions.find((competition) =>
                isPaperAndPosterCompetition(competition.name_competition)
            )

            if (!paperAndPoster) {
                setToast({
                    message: "Paper & Poster competition not found.",
                    type: "error",
                })
                return
            }

            setPaperAndPosterCompetition(paperAndPoster)
            fetchStagesByCompetition(paperAndPoster.id_competition)
        } catch (error) {
            console.error(error)
            setToast({
                message: "Failed to fetch Paper & Poster competition.",
                type: "error",
            })
        } finally {
            setIsLoading(false)
    ...
    const fetchRegistrations = async () => {
        try {
            ...
            const data = await res.json()
            setRegistrations(data.registrations || data.data || data || [])
        } catch (error) {
            console.error(error)
        }
    }
    ...
    const filteredTeams = useMemo(() => {
        ...
        const paperAndPosterRegistrations = registrations.filter(
            (registration) =>
                Number(registration.id_competition) ===
                Number(paperAndPosterCompetition.id_competition)
        )

        const registeredTeamLeaderIds = paperAndPosterRegistrations
            .map((registration) => Number(registration.id_team_leader))
            .filter(Boolean)

        return teams.filter((team) =>
            registeredTeamLeaderIds.includes(Number(team.id_team_leader))
        )
    }, [teams, registrations, paperAndPosterCompetition])
    ...
    const handleSubmitStage = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const body = editingStage
                ? {
                    id_competition:
                        paperAndPosterCompetition.id_competition,
                    stage_name: stageForm.stage_name,
                    start_stage: toISOStringWithTimezone(
                        stageForm.start_stage
                    ),
                    end_stage: toISOStringWithTimezone(stageForm.end_stage),
                    status_stage: stageForm.status_stage,
                    passed_teams: editingStage.passed_teams || [],
                }
                : {
                    id_competition:
                        paperAndPosterCompetition.id_competition,
            ...
            setEditingStage(null)
            fetchStagesByCompetition(paperAndPosterCompetition.id_competition)
        } catch (error) {
            console.error(error)
            setToast({
    ...
    const handleUpdatePassedTeams = async () => {
        if (!selectedStage || !paperAndPosterCompetition) return
            ...
            const res = await fetch(
                `${import.meta.env.VITE_API_BASE_URL}/updateStage/${selectedStage._id}`,
                {
                    ...
                    body: JSON.stringify({
                        ...
                        status_stage:
                            selectedStage.status_stage || "UPCOMING",
    ...
    }, [passedTeams, filteredTeams])

    return (
        <div className="text-white space-y-6 px-10 py-7">
            <div>
                <p className="text-2xl font-bold italic">
                    Paper & Poster Stage Management
                </p>
                <p className="text-sm opacity-70 mt-1">
                    Manage stages, schedules, status, and passed teams for Paper
                    & Poster Competition.
                </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="glass p-5 rounded-2xl">
                    ...
                    <p className="text-sm opacity-70">Registered Teams</p>
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-5">
                    <div>
                        ...
                        <p className="text-sm opacity-70">
                            Create, update, delete, and manage passed teams for
                            each stage.
                        </p>
                    </div>
                    ...
                    <button
                        ...
                        disabled={!paperAndPosterCompetition}
                        className="glass px-5 py-3 rounded-xl flex items-center gap-2 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        <Plus size={18} />
                        Add Stage
                    </button>
                ...
                <div className="overflow-x-auto">
                    ...
                    {!isLoading && stages.length === 0 && (
                        <p className="text-center text-sm opacity-70 py-6">
                            No stage data found for Paper & Poster Competition.
                        </p>
                    )}
            ...
            {isStageFormModalOpen && paperAndPosterCompetition && (
                ...
                <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-9999 px-5">
                    <div className="glass p-6 rounded-2xl w-full max-w-4xl max-h-[85vh] overflow-y-auto">
                        ...
                        {filteredTeams.length === 0 && (
                            <p className="text-center text-sm opacity-70 py-6">
                                No team data found for Paper & Poster
                                Competition.
                            </p>
                        )}
